...
def run_episode(env, agent, n_steps, device, callbacks=None):
    ...
    for idx in range(n_steps):
        ...
        step_memory = get_memory(state, decoded_action, reward, next_state, done, log_prob, mask, env.is_entry, env.log_return)
        step_extra_memory = get_extra_memory(env, decoded_action)
...
def get_memory(state, decoded_action, reward, next_state, done, log_prob, mask, is_entry, log_return):
    return [
            state,
